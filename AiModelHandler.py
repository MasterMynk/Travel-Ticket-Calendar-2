...
class Model:
    ...
    def _get_client(config: Configuration) -> genai.Client:
        ...
        try:
            with open(config.ai_model_credentials_json, "r") as credentials_json:
                return genai.Client(**json.loads(credentials_json.read()))
        except FileNotFoundError:
            log(LogLevel.Warning,
                f"{config.ai_model_credentials_json} doesn't exist. Required to use AI Model")
            raise
        except json.JSONDecodeError:
            log(LogLevel.Warning,
                f"{config.ai_model_credentials_json} is corrupted." "Must be of the format: {api_key: <YOUR_API_KEY_HERE>}")
            raise
    ...
    def parse(self: Self, ticket_fp: Path, prompt: str, config: Configuration) -> str:
        ...
        log(LogLevel.Status, f"Asking {config.ai_model} for help")
